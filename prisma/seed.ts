...
async function seed() {
  ...
  const RoskartografiaDZO = await db.file.create({
    data: {
      fileName:
        "ПОРЯДОК ВЗАИМОДЕЙСТВИЯ ОАО «Роскартография» с открытыми акционерными обществами, акциями которых владеет ОАО «Роскартография»",
  ...
  const Ukaz296 = await db.file.create({
    data: {
      fileName:
        'УКАЗ Президента РФ от 12 марта 2012 № 296 "ОБ ОТКРЫТОМ АКЦИОНЕРНОМ ОБЩЕСТВЕ "РОСКАРТОГРАФИЯ"',
  ...
  const lic1 = await db.file.create({
    data: {
      fileName:
        "Лицензия · на· осуществление· геодезических и картографических работ, 27-000001Ф от 31 марта 2015г. Переоформлена 21 марта 2017г. (Страница 1)",
      filePath: "lic 1.jpg",
      type: "lic",
    },
  });
  const lic2 = await db.file.create({
    data: {
      fileName:
        "Лицензия · на· осуществление· геодезических и картографических работ, 27-000001Ф от 31 марта 2015г. Переоформлена 21 марта 2017г. (Страница 2)",
      filePath: "lic 2.jpg",
      type: "lic",
    },
  });
  const lic3 = await db.file.create({
    data: {
      fileName:
        "Приложение к лицензии 27-000001Ф от 31 марта 2015 г.  Переоформлена 21 марта 2017г. (Страница 1)",
      filePath: "lic 3.jpg",
      type: "lic",
    },
  });
  const lic4 = await db.file.create({
    data: {
      fileName:
        "Приложение к лицензии 27-000001Ф от 31 марта 2015 г. (Страница 2)",
      filePath: "lic 4.jpg",
      type: "lic",
    },
  });
  const lic5 = await db.file.create({
    data: {
      fileName:
        "Свидетельство о допуске к определенному виду или видам работ, которые оказывают влияние на безопасность объектов капитального строительства, 01-И-№ 0763-4 от 15 июля 2015г. (Страница 1)",
      filePath: "lic 5.jpg",
      type: "lic",
    },
  });
  const lic6 = await db.file.create({
    data: {
      fileName:
        "Свидетельство о допуске к определенному виду или видам работ, которые оказывают влияние на безопасность объектов капитального строительства, 01-И-№ 0763-4 от 15 июля 2015г. (Страница 2)",
      filePath: "lic 6.jpg",
      type: "lic",
    },
  });
  const lic7 = await db.file.create({
    data: {
      fileName:
        "Свидетельство о допуске к определенному виду или видам работ, которые оказывают влияние на безопасность объектов капитального строительства, 01-И-№ 0763-4 от 15 июля 2015г. (Страница 3)",
      filePath: "lic 7.jpg",
      type: "lic",
    },
  });
  const lic8 = await db.file.create({
    data: {
      fileName:
        "Лицензия на осуществление работ, связанных с использованием сведений, составляющих государственную тайну (степень секретности СС) ГТ №0074248 от 01 июня 2016г.",
      filePath: "lic 8.jpg",
      type: "lic",
    },
  });
  const lic9 = await db.file.create({
    data: {
      fileName:
        "Сертификат соответствия требованиям ГОСТ Р ИСО 9001-2011 (ISO 9001:2011).",
      filePath: "lic 9.jpg",
      type: "lic",
    },
  });
  const lic10 = await db.file.create({
    data: {
      fileName:
        "Сертификат соответствия требованиям ГОСТ Р ИСО 9001-2011 (ISO 9001:2011) (2).",
      filePath: "lic 10.jpg",
      type: "lic",
    },
  });
  const lic11 = await db.file.create({
    data: {
      fileName:
        "Аттестат аккредитации в области обеспечения единства измерений   -  признание компетентности выполнять работы и оказывать услуги по поверке средств измерений.",
      filePath: "lic 11.jpg",
      type: "lic",
    },
  });
  const lic12 = await db.file.create({
    data: {
      fileName:
        "Аттестат аккредитации в области обеспечения единства измерений   -  признание компетентности выполнять работы и оказывать услуги по поверке средств измерений. (2)",
      filePath: "lic 12.jpg",
      type: "lic",
    },
  });
  ...
  const lic14 = await db.file.create({
    data: {
      fileName:
        "Сводная ведомость результатов проведения специальной оценки условий труда. (2)",
      filePath: "lic 14.jpg",
      type: "lic",
    },
  });
  const lic15 = await db.file.create({
    data: {
      fileName:
        "Сводная ведомость результатов проведения специальной оценки условий труда. (3)",
      filePath: "lic 15.jpg",
      type: "lic",
    },
  });
  ...
  const about_g: { link: string; text: string }[] = [
    { link: "contacts", text: "Отправить сообщение" },
    { link: "history", text: "История предприятия" },
    { link: "legislation", text: "Законодательные основы" },
    { link: "licenses", text: "Лицензии" },
    { link: "obshaya-info", text: "Общая информация" },
    { link: "phones", text: "Руководство \"ДВ АГП\"" },
    { link: "photogallery", text: "Фотоальбом" },
    { link: "rekvizits", text: "Реквизиты предприятия" },
    { link: "structura", text: "Структура предприятия" },
    { link: "tech", text: "Техническое оснащение" },
    { link: "vacancy", text: "Вакансии" },
  ];
  for (let index = 0; index < about_g.length; index++){
    let data = await db.sLink.create({
      data: {
        link: about_g[index].link,
        text: about_g[index].text,
        parent: {
          connectOrCreate: {
            where: {
              link: "about",
  ...
  const vidy = await db.sLink.create({
    data:{
        link: "vidy",
        text: "Виды деятельности",
    }
  })
  const katalog = await db.sLink.create({
    data:{
        link: "katalog",
        text: "Каталог продукции",
    }
  })
  const skhema = await db.sLink.create({
    data:{
        link: "skhema",
        text: "Схема зон ответственности",
    }
  })
  const guestbook = await db.sLink.create({
    data:{
        link: "guestbook",
        text: "Отправить сообщение",
    }
  })
  const payservice = await db.sLink.create({
    data:{
        link: "payservice",
        text: "Платные услуги",
    }
  })
  const raznoe = await db.sLink.create({
    data:{
        link: "raznoe",
        text: "Разное",
    }
  })
  const zakupki_g: {link: string, text: string}[] = [
    {link: "activnie_zakupki", text: "Активные закупки"},
    {link: "otchet_o_zakupkah", text: "Квартальные отчеты о закупках"},
    {link: "pologenie_o_zakupkah", text: "Положение о закупках"},
  ]
  for (let index = 0; index < zakupki_g.length; index++){
    let data = await db.sLink.create({
        data:{
            link: zakupki_g[index].link,
            text: zakupki_g[index].text,
            parent: {
                connectOrCreate: {
                    where: {
                        link: "zakupki"
  ...
  const news = await db.sLink.create({
    data:{
        link: "news",
        text: "Новости",
    }
  })
  const noprofilat = await db.sLink.create({
    data:{
        link: "noprofilat",
